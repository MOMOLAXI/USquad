...
public class BattleRoom
{
	...
	public void UpdateRoom()
	{
		...
		{
			string enemyLocation = _entityLocations[Random.Range(0, 4)];
			...
			{
				// 生成敌人实体
				SpawnHandle handle = _entitySpawner.SpawnSync(enemyLocation, _roomRoot.transform, spawnPosition, spawnRotation);
				EntityEnemy entity = handle.GameObj.GetComponent<EntityEnemy>();
			...
			else
			{
				// 生成小行星实体
				SpawnHandle handle = _entitySpawner.SpawnSync(enemyLocation, _roomRoot.transform, spawnPosition, spawnRotation);
				EntityAsteroid entity = handle.GameObj.GetComponent<EntityAsteroid>();
				entity.InitEntity(handle);
			}
	...
	public IEnumerator LoadRoom()
	{
		...
		// 加载背景音乐
		_musicHandle = AssetSystem.LoadAssetAsync<AudioClip>("music_background");
		yield return _musicHandle;

		// 播放背景音乐
		AudioSource audioSource = _roomRoot.AddComponent<AudioSource>();
		...
		// 创建玩家实体对象
		SpawnHandle handle = _entitySpawner.SpawnSync("player_ship", _roomRoot.transform);
		EntityPlayer entity = handle.GameObj.GetComponent<EntityPlayer>();
		entity.InitEntity(handle);
	...
	private void OnHandleEventMessage(IEventMessage message)
	{
		...
		{
			BattleEventDefine.PlayerDead msg = message as BattleEventDefine.PlayerDead;

			// 创建爆炸效果
			SpawnHandle handle = _entitySpawner.SpawnSync("explosion_player", _roomRoot.transform, msg.Position, msg.Rotation);
			EntityEffect entity = handle.GameObj.GetComponent<EntityEffect>();
			entity.InitEntity(handle);
		...
		{
			BattleEventDefine.EnemyDead msg = message as BattleEventDefine.EnemyDead;

			// 创建爆炸效果
			SpawnHandle handle = _entitySpawner.SpawnSync("explosion_enemy", _roomRoot.transform, msg.Position, msg.Rotation);
			EntityEffect entity = handle.GameObj.GetComponent<EntityEffect>();
			entity.InitEntity(handle);
		...
		{
			BattleEventDefine.AsteroidExplosion msg = message as BattleEventDefine.AsteroidExplosion;

			// 创建爆炸效果
			SpawnHandle handle = _entitySpawner.SpawnSync("explosion_asteroid", _roomRoot.transform, msg.Position, msg.Rotation);
			EntityEffect entity = handle.GameObj.GetComponent<EntityEffect>();
			entity.InitEntity(handle);
		...
		{
			BattleEventDefine.PlayerFireBullet msg = message as BattleEventDefine.PlayerFireBullet;

			// 创建子弹实体
			SpawnHandle handle = _entitySpawner.SpawnSync("player_bullet", _roomRoot.transform, msg.Position, msg.Rotation);
			EntityBullet entity = handle.GameObj.GetComponent<EntityBullet>();
			entity.InitEntity(handle);
		}
		else if (message is BattleEventDefine.EnemyFireBullet)
		{
			BattleEventDefine.EnemyFireBullet msg = message as BattleEventDefine.EnemyFireBullet;

			// 创建子弹实体
			SpawnHandle handle = _entitySpawner.SpawnSync("enemy_bullet", _roomRoot.transform, msg.Position, msg.Rotation);
			EntityBullet entity = handle.GameObj.GetComponent<EntityBullet>();
			entity.InitEntity(handle);
		}
	}
