...
namespace UniverseEngine.Editor
{
	internal class ReporterSummaryViewer
	{
		...
		{
			// 加载布局文件
			m_VisualAsset = WindowModule.LoadWindowUxml<ReporterSummaryViewer>();
		...
		public void FillViewData(BuildReport buildReport)
		{
			...
			m_Items.Add(new("YooAsset版本", buildReport.Summary.AssetSystemVersion));
		...
		static string ConvertTime(double time)
		{
			if (time <= 60)
			{
				return $"{time}秒钟";
		...
		{
			return size == 0 ? "0" : EditorUtility.FormatBytes(size);
		}
	}
}
#endif
