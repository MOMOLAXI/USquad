...
namespace UniverseEngine.Editor
{
	...
	public class ShaderVariantCollectionManifest
	{
		...
		public static ShaderVariantCollectionManifest Extract(ShaderVariantCollection svc)
		{
			...
			{
				ShaderTotalCount = ShaderModule.GetCurrentShaderVariantCollectionShaderCount(),
				VariantTotalCount = ShaderModule.GetCurrentShaderVariantCollectionVariantCount()
			};

			using (SerializedObject so = new(svc))
			{
				SerializedProperty shaderArray = so.FindProperty("m_Shaders.Array");
				if (shaderArray is { isArray: true })
				{
					for (int i = 0; i < shaderArray.arraySize; ++i)
					{
						...
						if (shaderRef is { propertyType: SerializedPropertyType.ObjectReference } && shaderVariantsArray is { isArray: true })
						{
