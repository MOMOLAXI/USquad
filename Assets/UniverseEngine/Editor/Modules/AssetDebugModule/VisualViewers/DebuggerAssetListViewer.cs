...
namespace UniverseEngine.Editor
{
	internal class DebuggerAssetListViewer
	{
		...
		{
			// 加载布局文件		
			m_VisualAsset = WindowModule.LoadWindowUxml<DebuggerAssetListViewer>();
			...
			m_AssetListView.selectionChanged += AssetListView_onSelectionChange;
		...
		List<DebugProviderInfo> FilterViewItems(DebugReport debugReport, string searchKeyWord)
		{
		...
		// 顶部列表相关
		VisualElement MakeAssetListViewItem()
		{
		...
		void BindAssetListViewItem(VisualElement element, int index)
		{
			...
			// Status
			StyleColor textColor = providerInfo.Status == ProviderBase.EStatus.Failed.ToString() ? new(Color.yellow) : label1.style.color;
			Label label6 = element.Q<Label>("Label6");
			label6.text = providerInfo.Status;
			label6.style.color = textColor;
		}
		
		void AssetListView_onSelectionChange(IEnumerable<object> objs)
		{
		...
		// 底部列表相关
		static VisualElement MakeDependListViewItem()
		{
			VisualElement element = new()
			{
				style =
		...
		void BindDependListViewItem(VisualElement element, int index)
		{
		...
		void FillDependListView(DebugProviderInfo selectedProviderInfo)
		{
