...
namespace UniverseEngine.Editor
{
    ...
    public class EntityInspector : UnityEditor.Editor
    {
        ...
        void InitData()
        {
            ...
            m_Igo = target as Entity;
            if (m_Igo == null)
            {
                return;
            }

            EntityID objectID = m_Igo.ID;
        ...
        static string ShowTextField(Var var)
        {
            switch (var.VariableType)
            {
                ...
                case VarType.Object: return var.GetEntityID().ToString();
        ...
        void DrawSetPropArea()
        {
            ...
            if (DrawHeader("SetPropArea(设置/添加 属性)"))
            {
                ...
                if (GUILayout.Button("SetProp"))
                {
                    ...
                    {
                        EditorLog.Error("EntityEditor:DrawSetPropArea. PropName or PropValue is empty");
                        return;
                    }

                    EditorLog.Info($"EntityEditor:DrawSetPropArea. Set prop finish. EntityID:{m_Igo.ID}, Name:{s_PropName}, Value:{s_PropValue}");

                    switch (s_VarType)
                    {
                        case VarType.Bool:
                            m_Igo.ID.SetProp(s_PropName, new(s_PropValue.ToBool()));
                            break;
                        case VarType.Int32:
                            m_Igo.ID.SetProp(s_PropName, new(s_PropValue.ToInt()));
                            break;
                        case VarType.Int64:
                            m_Igo.ID.SetProp(s_PropName, new(s_PropValue.ToInt64()));
                            break;
                        case VarType.Float:
                            m_Igo.ID.SetProp(s_PropName, new(s_PropValue.ToFloat()));
                            break;
                        case VarType.String:
                            m_Igo.ID.SetProp(s_PropName, new(s_PropValue));
                            break;
                        ...
                        case VarType.Max:
                        default:
                            EditorLog.Error($"EntityEditor:DrawSetPropArea. Type {s_VarType} Unprocessed");
                            break;
                    }
                }
                EditorGUILayout.EndVertical();
