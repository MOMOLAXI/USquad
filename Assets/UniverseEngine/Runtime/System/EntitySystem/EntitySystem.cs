...
namespace UniverseEngine
{
    ...
    internal class EntitySystem : EngineSystem
    {
        ...
        /// <returns></returns>
        public Entity CreateEntity(string className, int identity, Entity parent = null, bool isStatic = false)
        {
            ...
            entity.SetString("ClassName", className);
            entity.SendClassEvent(EntityEvent.OnCreate, id, Args.Empty);
            if (parent != null)
            {
        ...
        Entity CreateRoot()
        {
            GameObject go = UniverseEngine.CreateGlobalGameObject(nameof(EntitySystem));
            Entity root = go.AddComponent<Entity>();
            root.ID = new(int.MaxValue, int.MaxValue);
