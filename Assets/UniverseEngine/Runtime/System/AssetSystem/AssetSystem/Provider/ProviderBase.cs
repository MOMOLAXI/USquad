...
namespace UniverseEngine
{
	internal abstract class ProviderBase
	{
		...
		private readonly List<OperationHandleBase> _handles = new();
		...
		public ProviderBase(AssetSystemImpl impl, string providerGUID, AssetInfo assetInfo)
		{
			...
			if (impl != null)
			{
				...
				List<BundleLoaderBase> dependBundles = impl.CreateDependAssetBundleLoaders(assetInfo);
		...
		{
			if (RefCount <= 0)
				Log<AssetSystem>.Warning("Asset provider reference count is already zero. There may be resource leaks !");
		...
		public void WaitForAsyncComplete()
		{
			...
			if (IsDone == false)
			{
				Log<AssetSystem>.Warning($"WaitForAsyncComplete failed to loading : {MainAssetInfo.AssetPath}");
			}
		}
		...
		protected void InvokeCompletion()
		{
			...
			List<OperationHandleBase> tempers = new(_handles);
		...
		internal DownloadReport GetDownloadReport()
		{
			...
			foreach (BundleLoaderBase dependBundle in DependBundleGroup.DependBundles)
			{
		...
		{
			DebugBundleInfo bundleInfo = new DebugBundleInfo();
