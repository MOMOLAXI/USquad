...
namespace UniverseEngine
{
	...
	{
		private readonly Dictionary<string, BundleLoaderBase> _loaderDic = new(5000);
		private readonly List<BundleLoaderBase> _loaderList = new(5000);

		private readonly Dictionary<string, ProviderBase> _providerDic = new(5000);
		private readonly List<ProviderBase> _providerList = new(5000);

		private readonly static Dictionary<string, SceneOperationHandle> _sceneHandles = new(100);
		...
		public IAssetDecryption DecryptionServices { private set; get; }
		...
		public void Initialize(string packageName, bool simulationOnEditor, long loadingMaxTimeSlice, int downloadFailedTryAgain,
			IAssetDecryption decryptionServices, IBundleServices bundleServices)
		{
		...
		public void Update()
		{
			...
			// 更新加载器	
			foreach (BundleLoaderBase loader in _loaderList)
			{
				loader.Update();
			}
		...
		{
			foreach (ProviderBase provider in _providerList)
			{
				provider.Destroy();
			}
			_providerList.Clear();
		...
		public void UnloadUnusedAssets()
		{
			...
			{
				Log<AssetSystem>.Warning("Can not unload unused assets when processing resource loading !");
				return;
			}
		...
		public SceneOperationHandle LoadSceneAsync(AssetInfo assetInfo, LoadSceneMode sceneMode, bool activateOnLoad, int priority)
		{
			...
			{
				Log<AssetSystem>.Error($"Failed to load scene ! {assetInfo.Error}");
		...
		public AssetOperationHandle LoadAssetAsync(AssetInfo assetInfo)
		{
			if (assetInfo.IsInvalid)
			{
				Log<AssetSystem>.Error($"Failed to load asset ! {assetInfo.Error}");
				CompletedProvider completedProvider = new(assetInfo);
				completedProvider.SetCompleted(assetInfo.Error);
		...
		public SubAssetsOperationHandle LoadSubAssetsAsync(AssetInfo assetInfo)
		{
			if (assetInfo.IsInvalid)
			{
				Log<AssetSystem>.Error($"Failed to load sub assets ! {assetInfo.Error}");
				CompletedProvider completedProvider = new(assetInfo);
				completedProvider.SetCompleted(assetInfo.Error);
		...
		public RawFileOperationHandle LoadRawFileAsync(AssetInfo assetInfo)
		{
			if (assetInfo.IsInvalid)
			{
				Log<AssetSystem>.Error($"Failed to load raw file ! {assetInfo.Error}");
				CompletedProvider completedProvider = new(assetInfo);
				completedProvider.SetCompleted(assetInfo.Error);
		...
		{
			// 释放所有场景句柄
			foreach (KeyValuePair<string, SceneOperationHandle> valuePair in _sceneHandles)
			{
		...
		{
			List<DebugProviderInfo> result = new(_providerList.Count);
			foreach (ProviderBase provider in _providerList)
			{
				...
				providerInfo.DependBundleInfos = new();
