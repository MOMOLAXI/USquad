...
namespace UniverseEngine
{
	internal abstract class BundleLoaderBase
	{
		...
		private readonly List<ProviderBase> _providers = new(100);
		...
		public bool CanDestroy()
		{
			...
			// 检查引用链上的资源包是否已经全部销毁
			// 注意：互相引用的资源包无法卸载！
			foreach (int bundleID in MainBundleInfo.Bundle.ReferenceIDs)
			{
		...
		public virtual void Destroy(bool forceDestroy)
		{
			...
			{
				if (RefCount > 0)
					throw new($"Bundle file loader ref is not zero : {MainBundleInfo.Bundle.BundleName}");
				if (IsDone() == false)
					throw new($"Bundle file loader is not done : {MainBundleInfo.Bundle.BundleName}");
			}
