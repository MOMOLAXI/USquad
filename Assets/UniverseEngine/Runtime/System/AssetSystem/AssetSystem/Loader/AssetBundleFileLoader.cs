	...
	internal sealed class AssetBundleFileLoader : BundleLoaderBase
	{
		...
		public override void Update()
		{
			...
			if (_steps == ESteps.Unpack)
			{
				...
				BundleInfo bundleInfo = MainBundleInfo.Bundle.GetUnpackInfo();
			...
			if (_steps == ESteps.LoadFile)
			{
				...
				if (System.IO.File.Exists(FileLoadPath) == false)
				{
					...
					Log<AssetSystem>.Error(LastError);
					return;
				}
			#endif
				...
				else
				{
					if (Impl.DecryptionServices == null)
					{
						_steps = ESteps.Done;
						Status = EStatus.Failed;
						LastError = $"{nameof(IAssetDecryption)} is null : {MainBundleInfo.Bundle.BundleName}";
						Log<AssetSystem>.Error(LastError);
						return;
					}

					AssetBundleDecryptFileInfo fileInfo = new();
			...
			if (_steps == ESteps.CheckLoadFile)
			{
				if (_createRequest != null)
				{
					...
					{
						// 强制挂起主线程（注意：该操作会很耗时）
						Log<AssetSystem>.Warning("Suspend the main thread to load unity bundle.");
				...
				if (CacheBundle == null)
				{
					...
					{
						EAssetFileCacheResult result = AssetCacheSystem.VerifyingCacheFile(MainBundleInfo.Bundle.PackageName, MainBundleInfo.Bundle.CacheGuid);
						if (result != EAssetFileCacheResult.Succeed)
						{
							Log<AssetSystem>.Error($"Found possibly corrupt file ! {MainBundleInfo.Bundle.CacheGuid}");
							AssetCacheSystem.RemoveCacheFile(MainBundleInfo.Bundle.PackageName, MainBundleInfo.Bundle.CacheGuid);
						}
					}
				}
				else
			...
			while (true)
			{
				...
				if (frame == 0)
				{
					if (_isShowWaitForAsyncError == false)
					{
						...
						Log<AssetSystem>.Error($"{nameof(WaitForAsyncComplete)} failed ! Try load bundle : {MainBundleInfo.Bundle.BundleName} from remote with sync load method !");
					}
					break;
				}
