...
namespace UniverseEngine
{
	public abstract class DownloaderOperation : AsyncOperationBase
	{
		...
		internal DownloaderOperation(List<BundleInfo> downloadList, int downloadingMaxNumber, int failedTryAgain, int timeout)
		{
			...
			if (downloadList != null)
			{
				...
				foreach (BundleInfo packageBundle in downloadList)
				{
		...
		{
			Log<AssetSystem>.Info($"Begin to download : {TotalDownloadCount} files and {TotalDownloadBytes} bytes");
			...
			if (m_Steps == ESteps.Loading)
			{
				...
				if (m_DownloadList.Count > 0 && m_FailedList.Count == 0)
				{
					...
					if (m_Downloaders.Count < m_DownloadingMaxNumber)
					{
						...
						DownloaderBase operation = DownloadSystem.BeginDownload(bundleInfo, m_FailedTryAgain, m_Timeout);
	...
	public sealed class ResourceUnpackerOperation : DownloaderOperation
	{
		...
		{
			List<BundleInfo> downloadList = new();
			ResourceUnpackerOperation operation = new ResourceUnpackerOperation(downloadList, upackingMaxNumber, failedTryAgain, int.MaxValue);
			return operation;
		}
	}
}