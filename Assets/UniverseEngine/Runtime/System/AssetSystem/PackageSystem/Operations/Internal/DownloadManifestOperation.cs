	...
	internal class DownloadManifestOperation : AsyncOperationBase
	{
		...
		internal override void Update()
		{
			...
			if (m_Steps == ESteps.DownloadPackageHashFile)
			{
				...
				{
					string savePath = FileSystem.GetCachedPackageHashFilePath(m_PackageName, m_PackageVersion);
					string fileName = FileSystem.GetPackageHashFileName(m_PackageName, m_PackageVersion);
					string webURL = GetDownloadRequestURL(fileName);
					Log<AssetSystem>.Info($"Beginning to download package hash file : {webURL}");
					m_Downloader1 = new();
			...
			if (m_Steps == ESteps.DownloadManifestFile)
			{
				...
				{
					string savePath = FileSystem.GetCachedManifestFilePath(m_PackageName, m_PackageVersion);
					string fileName = FileSystem.GetManifestBinaryFileName(m_PackageName, m_PackageVersion);
					string webURL = GetDownloadRequestURL(fileName);
					Log<AssetSystem>.Info($"Beginning to download manifest file : {webURL}");
					m_Downloader2 = new();
