...
namespace UniverseEngine
{
    public partial struct UniTask
    {
        ...
        {
            return new(WaitUntilPromise.Create(predicate, timing, cancellationToken, out var token), token);
        }
        ...
        {
            return new(WaitWhilePromise.Create(predicate, timing, cancellationToken, out var token), token);
        }
        ...
        {
            return new(WaitUntilCanceledPromise.Create(cancellationToken, timing, out var token), token);
        }
            ...
            where T : class
        {
            ...
            public static IUniTaskSource<U> Create(T target, Func<T, U> monitorFunction, IEqualityComparer<U> equalityComparer, PlayerLoopTiming timing, CancellationToken cancellationToken, out short token)
            {
                ...
                result.target = new(target, false); // wrap in WeakReference.
                result.monitorFunction = monitorFunction;
                result.currentValue = monitorFunction(target);
                result.equalityComparer = equalityComparer ?? UnityEqualityComparer.GetDefault<U>();
                result.cancellationToken = cancellationToken;
