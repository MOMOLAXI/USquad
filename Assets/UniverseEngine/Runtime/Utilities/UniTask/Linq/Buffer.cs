...
namespace UniverseEngine
{
    ...
    sealed internal class Buffer<TSource> : IUniTaskAsyncEnumerable<IList<TSource>>
    {
    ...
    sealed internal class BufferSkip<TSource> : IUniTaskAsyncEnumerable<IList<TSource>>
    {
        readonly IUniTaskAsyncEnumerable<TSource> source;
        readonly int count;
        readonly int skip;
            ...
            static void MoveNextCore(object state)
            {
                ...
                if (self.TryGetResult(self.awaiter, out var result))
                {
                    if (result)
                    {
                        ...
                        {
                            self.buffers.Enqueue(new(self.count));
                        }
