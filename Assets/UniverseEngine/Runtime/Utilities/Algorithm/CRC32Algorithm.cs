...
namespace UniverseEngine
{
	...
	{
		uint m_CurrentCrc;
		static readonly SafeProxy s_Proxy = new();
		...
		static uint AppendInternal(uint initial, byte[] input, int offset, int length)
		{
			if (length > 0)
			{
				return s_Proxy.Append(initial, input, offset, length);
			}
